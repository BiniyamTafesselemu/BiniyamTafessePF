...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const phoneRegex = /^\+?[0-9]{10,}$/; // Allows a '+' followed by at least 10 digits

    if (formData.phone && !phoneRegex.test(formData.phone)) {
      setFeedback({ type: "error", message: "Phone number must be in a valid format (e.g., +251912324389)." });
      setIsLoading(false);
      return;
    }
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-8 lg:items-center">
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <label htmlFor="telegram" className="block text-sm font-medium mb-2">Telegram Username</label>
                <input
                  type="text"
                  id="telegram"
                  name="telegram"
